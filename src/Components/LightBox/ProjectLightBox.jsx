...
const ProjectLightBox = ({ photos, viewerIsOpen, currentImage, setCurrentImage, onClose, title_image }) => {
    if (!photos || photos.length === 0) {
        // Handle the case when photos is undefined or empty.
        return null;
    }
    // Create an array to include title_image at index 0
    const lightboxPhotos = title_image ? [{ name: title_image, alt: 'Title Image' }, ...photos] : photos;
    ...
    return (
        <Lightbox
            images={lightboxPhotos.map(photo => ({ src: photo.name, alt: photo.alt }))}
